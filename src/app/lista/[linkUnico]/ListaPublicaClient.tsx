...
export default function ListaPublicaClient({
  ...
  const ordenadas = useMemo(() => {
    ...
    copia.sort((a, b) => (a.destacada === b.destacada ? 0 : a.destacada ? -1 : 1));
    return copia;
  }, [papelarias, filtro]);
      ...
      {papelarias && (
        <>
          <div className="mb-4 flex gap-2 text-sm">
            ...
            <button onClick={() => setFiltro("preco")} className={filtro === "preco" ? "font-bold text-violet-700" : "text-gray-600"}>
              Menor preço
            </button>
            ·
            <button onClick={() => setFiltro("proxima")} className={filtro === "proxima" ? "font-bold text-violet-700" : "text-gray-600"}>
              Mais próxima
            </button>
            ·
            <button onClick={() => setFiltro("disponibilidade")} className={filtro === "disponibilidade" ? "font-bold text-violet-700" : "text-gray-600"}>
          ...
          <ul className="mb-6 flex flex-col gap-2">
            {ordenadas.map((p) => (
              <li
                ...
                className={`relative cursor-pointer rounded-2xl border-2 p-3 ${
                  p.destacada ? "border-orange-300 bg-orange-50" : papelariaSelecionada === p.papelariaId ? "border-violet-400 bg-violet-50" : "border-violet-100"
                }`}
              >
                <div className="flex items-center justify-between">
                  <p className="font-medium">
                    {p.destacada && <span className="badge mr-2" style={{ background: "#ffedd5", color: "#c2410c" }}>⭐ Destaque</span>}
                    {p.nome}
                  </p>
          ...
          {papelaria && !resultadoOrcamento && (
            <form onSubmit={handleSubmitOrcamento} className="card p-4">
              ...
              <div className="flex flex-col gap-3">
                ...
                <label className="label">
                  ...
                  <input name="responsavelTelefone" required placeholder="5511999999999" className="input" />
                </label>
                <label className="label">
                  Nome do aluno(a)
                  <input name="nomeAluno" className="input" />
                </label>
                <p className="text-xs text-gray-400">
          ...
          {resultadoOrcamento && (
            <div className="rounded-md border border-green-300 bg-green-50 p-4">
              ...
              <p className="mb-3 text-sm text-gray-600">
                A papelaria já recebeu seu pedido. Converse com ela e acompanhe o status sem
                precisar trocar telefone — tudo fica registrado aqui dentro.
              </p>
